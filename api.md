...
- ✅ Suporte multi-brand (apostatudo, geralbet, bullsbet, sortenabet)
- ✅ Depósitos via PIX
- ✅ Iniciar jogos
...
- `X-Brand-Slug: <brand>` - Slug da marca (apostatudo, geralbet, etc)
...
- `brand_slug` (string, obrigatório): Slug da marca (apostatudo, geralbet, bullsbet, sortenabet)
...
curl -X POST https://routes-eb.grupoautoma.com/api/auth/login \
  ...
  -d '{
    "email": "automabets@gmail.com",
    "password": "eOcMPuLRns73DI@",
    "brand_slug": "apostatudo"
  }'
```
...
**Exemplo de Requisição:**
```bash
curl -X GET "http://localhost:8075/api/auth/user?collection=users_apostatudo" \
  -H "Authorization: Bearer seu-token-aqui" \
  -H "X-Brand-Slug: apostatudo"
```
...
**Exemplo de Requisição:**
```bash
curl -X GET "http://localhost:8075/api/users/users_apostatudo?limit=10&skip=0" \
  -H "Content-Type: application/json"
```
  ...
  "users": [
    {
      ...
      "collection_name": "users_apostatudo"
    }
  ],
  "pagination": {
    "total": 1,
...
**Exemplo de Requisição:**
```bash
curl -X GET "http://localhost:8075/api/users/users_apostatudo/usuario@example.com" \
  -H "Content-Type: application/json"
```
...
curl -X POST https://routes-eb.grupoautoma.com/api/register \
  ...
  -d '{
    "number": "03276799561",
    "phone": "71996640989",
    "email": "grupoautoma@gmail.com",
    "password": "Teste@123456",
    "affiliation_code": "apostatudo",
...
{
  "success": true,
  "transaction_id": "apostatudobetbr_11361812",
...
curl -X GET "https://routes-eb.grupoautoma.com/api/auth/user?collection=users_apostatudo" \
...
4. **Brands Suportadas:** apostatudo, geralbet, bullsbet, sortenabet
