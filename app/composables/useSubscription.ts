...
const applySubscriptionState = (email: string | null, active: boolean, role: 'paid' | 'free') => {
  subscriptionState.isSubscribed = active
  subscriptionState.role = role
  subscriptionState.email = email
  subscriptionState.showModal = !active
...
const saveCache = (email: string, active: boolean, role: 'paid' | 'free') => {
  if (!import.meta.client) return
  sessionStorage.setItem(SUBSCRIPTION_SESSION_KEY, JSON.stringify({ active, role, email, ts: Date.now() }))
}
...
export const useSubscription = () => {
  ...
  const init = async (email?: string | null, options: { force?: boolean } = {}) => {
    ...
    if (hasFreshMatchingCache) {
      applySubscriptionState(cache.email, cache.active, cache.role ?? (cache.active ? 'paid' : 'free'))
      return cache.active
    }
  ...
  const checkSubscription = async (email: string): Promise<boolean> => {
    ...
    error.value = ''
    try {
      const result = await $fetch<{ active: boolean; role: 'paid' | 'free' }>('/api/subscription/check', {
        params: { email }
      })
      const role = result.role ?? (result.active ? 'paid' : 'free')
      saveCache(email, result.active, role)
      applySubscriptionState(email, result.active, role)
